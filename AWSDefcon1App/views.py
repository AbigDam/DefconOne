...
def devLog(request):
      return render(request, "AWSDefcon1App/devLog.html")
  
def error500(request):
        return bad_request(request, title="A serious issue occured", message="The database is currently expiriencing multiple requests at once, this will likely fix itself automatically if you try again")
...
def full_index(request):
    ...
    for game in games_list:
        nations = list(Nations.objects.filter(game=game))
        nations.sort(key=lambda n: 0 if n.user.username == 'empty' else 1)
        games.append([game.id] + nations)  
...
def makealliance(request,game_id):
    ...
    if request.method == 'POST':
        ...
        if action == 'reject':
            ...
            if accepting_nation.alliance_name != "":
                player_nation.alliance_name = accepting_nation.alliance_name
                ...
                alliance_name = "Unnamed_" + str(uuid.uuid4())
                accepting_nation.alliance_name = alliance_name
                player_nation.alliance_name = alliance_name  
                Announcements.objects.create(text =f"{player_nation.name} has accepted {accepting_nation.name}'s invitation to an alliance", start_time = datetime.now(), game = Games.objects.get(id = game_id))              
            ...
            if nation2.user.username == 'empty' or nation2.user.username == 'closed':
              ...
              if chance == 1 or temp_hostility == 1:
                  ...
                  if player_nation.alliance_name != "":
                      alliance_name = player_nation.alliance_name
                  elif nation2.alliance_name != "":
                      alliance_name = nation2.alliance_name
                  else:
                      alliance_name = "Unnamed_" + str(uuid.uuid4())
                  
                  if "Unnamed_" in alliance_name:
                      un_named_alliance = True
                  ...
                  war = War.objects.filter(nation2 = player_nation, nation1 = nation2).delete()
                  yesman = True
                  selected_nation_alliance.delete()

            if yesman:
                if un_named_alliance:
                    Announcements.objects.create(text =f"{nation2.name} has accepted an alliance with {player_nation.name}", start_time = datetime.now(), game = Games.objects.get(id = game_id))
                else:
                     Announcements.objects.create(text =f"{nation2.name} has joined the {alliance_name}", start_time = datetime.now(), game = Games.objects.get(id = game_id))
            elif nation2.user.username == 'empty' or nation2.user.username == 'closed':
                if "Unnamed_" in player_nation.alliance_name or player_nation.alliance_name == "":
                    announcements = Announcements.objects.create(text =f"{nation2.name} has rejected {Nations.objects.get(user=request.user,game_id = game_id).name}'s invitation to an alliance", start_time = datetime.now(), game = Games.objects.get(id = game_id))
                else:
...
def send(request,game_id):
    ...
    if playernation.act_planes > playernation.planes:
        playernation.act_planes = playernation.planes

    if playernation.act_boats > playernation.boats:
        playernation.act_boats = playernation.boats

    if playernation.act_divisions > playernation.divisions:
        playernation.act_divisions = playernation.divisions
    playernation.save()
    return HttpResponseRedirect(reverse('map', kwargs={'game_id': game_id}))
...
def current_wars(request,game_id):
    if request.method == 'POST':
        ...
        if winner:
            ...
            return HttpResponseRedirect(reverse('loader', kwargs={'game_id': game_id, 'reload':1}))
...
def map(request, game_id):
    ...
    squares = list(
        Square.objects.filter(
            map=Map.objects.get(game=Games.objects.get(id=game_id))
        ).values(
            "number",
            "name",
            "neighbors",
...
def makegame(request,game_id):
    ...
    { "game": game_obj, "name": "Afghanistan", "user": closed, "player_number": 9, "states": 2, "divisions": 38, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 38, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Austria", "user": closed, "player_number": 0, "states": 4, "divisions": 36, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 36, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Australia", "user": closed, "player_number": 10, "states": 15, "divisions": 25, "boats": 50, "planes": 5000, "alliance_name": "Allies", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 25, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Argentina", "user": closed, "player_number": 11, "states": 14, "divisions": 34, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 34, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Albania", "user": closed, "player_number": 12, "states": 3, "divisions": 37, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 37, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Belgium", "user": closed, "player_number": 13, "states": 10, "divisions": 30, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 30, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Bulgaria", "user": closed, "player_number": 14, "states": 4, "divisions": 36, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 36, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Bhutan", "user": closed, "player_number": 15, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 10, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Bolivian Republic", "user": closed, "player_number": 16, "states": 2, "divisions": 38, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 38, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Brazil", "user": closed, "player_number": 17, "states": 31, "divisions": 28, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 28, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "British Malaya", "user": closed, "player_number": 18, "states": 2, "divisions": 38, "boats": 50, "planes": 5000, "alliance_name": "Allies", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 38, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "British Raj", "user": closed, "player_number": 19, "states": 26, "divisions": 14, "boats": 50, "planes": 5000, "alliance_name": "Allies", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 14, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Canada", "user": closed, "player_number": 20, "states": 23, "divisions": 17, "boats": 50, "planes": 5000, "alliance_name": "Allies", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 17, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "China", "user": closed, "player_number": 21, "states": 22, "divisions": 37, "boats": 50, "planes": 5000, "alliance_name": "Second United Front", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 37, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Chile", "user": closed, "player_number": 22, "states": 8, "divisions": 37, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 37, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Colombia", "user": closed, "player_number": 23, "states": 3, "divisions": 37, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 37, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Communist China", "user": closed, "player_number": 24, "states": 1, "divisions": 86, "boats": 50, "planes": 5000, "alliance_name": "Second United Front", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 2, "requests": 4, "act_divisions": 86, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Costa Rica", "user": closed, "player_number": 25, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 10, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Cuba", "user": closed, "player_number": 26, "states": 1, "divisions": 80, "boats": 200, "planes": 4000, "alliance_name": "", "points": 1, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 10, "act_divisions": 80, "act_boats": 200, "act_planes": 4000 },
    { "game": game_obj, "name": "Czechoslovakia", "user": closed, "player_number": 27, "states": 9, "divisions": 31, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 31, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Denmark", "user": closed, "player_number": 28, "states": 7, "divisions": 33, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 33, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Dutch East Indies", "user": closed, "player_number": 29, "states": 8, "divisions": 32, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 32, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Dominican Republic", "user": closed, "player_number": 30, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 10, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Ecuador", "user": closed, "player_number": 31, "states": 2, "divisions": 38, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 38, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "El Salvador", "user": closed, "player_number": 32, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Estonia", "user": closed, "player_number": 33, "states": 5, "divisions": 35, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 35, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Ethiopia", "user": closed, "player_number": 34, "states": 10, "divisions": 30, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 30, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Finland", "user": closed, "player_number": 35, "states": 12, "divisions": 27, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 27, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Guangxi Clique", "user": closed, "player_number": 36, "states": 5, "divisions": 35, "boats": 50, "planes": 5000, "alliance_name": "Second United Front", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 35, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Guatemala", "user": closed, "player_number": 37, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 10, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Haiti", "user": closed, "player_number": 38, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 10, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Honduras", "user": closed, "player_number": 39, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 10, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Iceland", "user": closed, "player_number": 40, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 10, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Iran", "user": closed, "player_number": 41, "states": 12, "divisions": 28, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 28, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Iraq", "user": closed, "player_number": 42, "states": 3, "divisions": 37, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 37, "act_boats": 50, "act_planes": 5000 },
    ...
    { "game": game_obj, "name": "Kingdom of Greece", "user": closed, "player_number": 44, "states": 5, "divisions": 33, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 33, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Kingdom of Hungary", "user": closed, "player_number": 45, "states": 3, "divisions": 37, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 37, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Latvia", "user": closed, "player_number": 46, "states": 5, "divisions": 35, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 35, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Liberia", "user": closed, "player_number": 47, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Lithuania", "user": closed, "player_number": 48, "states": 5, "divisions": 35, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 35, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Luxembourg", "user": closed, "player_number": 49, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Manchukuo", "user": closed, "player_number": 50, "states": 7, "divisions": 33, "boats": 50, "planes": 5000, "alliance_name": "GEACPS", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 33, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Mexico", "user": closed, "player_number": 51, "states": 13, "divisions": 27, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 27, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Mongolia", "user": closed, "player_number": 52, "states": 5, "divisions": 35, "boats": 50, "planes": 5000, "alliance_name": "Comintern", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 35, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Mengkukuo", "user": closed, "player_number": 53, "states": 2, "divisions": 38, "boats": 50, "planes": 5000, "alliance_name": "GEACPS", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 38, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Nepal", "user": closed, "player_number": 54, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 10, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Netherlands", "user": closed, "player_number": 55, "states": 4, "divisions": 35, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 35, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "New Zealand", "user": closed, "player_number": 56, "states": 3, "divisions": 37, "boats": 50, "planes": 5000, "alliance_name": "Allies", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 37, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Nicaragua", "user": closed, "player_number": 57, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 10, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Norway", "user": closed, "player_number": 58, "states": 11, "divisions": 29, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 29, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Oman", "user": closed, "player_number": 59, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 10, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Panama", "user": closed, "player_number": 60, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 10, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Peru", "user": closed, "player_number": 61, "states": 5, "divisions": 35, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 35, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Philippines", "user": closed, "player_number": 62, "states": 7, "divisions": 33, "boats": 50, "planes": 5000, "alliance_name": "Neutrality Pact", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 33, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Portugal", "user": closed, "player_number": 63, "states": 20, "divisions": 20, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 20, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Poland", "user": closed, "player_number": 64, "states": 18, "divisions": 22, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 22, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Romania", "user": closed, "player_number": 65, "states": 11, "divisions": 29, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 29, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Republic of Paraguay", "user": closed, "player_number": 66, "states": 2, "divisions": 38, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 38, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Siam", "user": closed, "player_number": 67, "states": 4, "divisions": 36, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 36, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Sinkiang", "user": closed, "player_number": 68, "states": 6, "divisions": 34, "boats": 50, "planes": 5000, "alliance_name": "Second United Front", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 34, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Saudi Arabia", "user": closed, "player_number": 69, "states": 9, "divisions": 31, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 31, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Shanxi", "user": closed, "player_number": 70, "states": 3, "divisions": 37, "boats": 50, "planes": 5000, "alliance_name": "Second United Front", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 37, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "South Africa", "user": closed, "player_number": 71, "states": 7, "divisions": 33, "boats": 50, "planes": 5000, "alliance_name": "Allies", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 33, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Spain", "user": closed, "player_number": 72, "states": 24, "divisions": 14, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 14, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Sweden", "user": closed, "player_number": 73, "states": 13, "divisions": 27, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 27, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Switzerland", "user": closed, "player_number": 74, "states": 5, "divisions": 35, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 35, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Sultanate of Aussa", "user": closed, "player_number": 75, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 10, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Tannu Tuva", "user": closed, "player_number": 76, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "Comintern", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 10, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Turkey", "user": closed, "player_number": 77, "states": 21, "divisions": 19, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 19, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Tibet", "user": closed, "player_number": 78, "states": 3, "divisions": 37, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 37, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Uruguay", "user": closed, "player_number": 79, "states": 3, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Venezuela", "user": closed, "player_number": 80, "states": 3, "divisions": 37, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 37, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Xibei San Ma", "user": closed, "player_number": 81, "states": 7, "divisions": 33, "boats": 50, "planes": 5000, "alliance_name": "Second United Front", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 33, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Yugoslavia", "user": closed, "player_number": 82, "states": 14, "divisions": 26, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 26, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Yunnan", "user": closed, "player_number": 83, "states": 2, "divisions": 38, "boats": 50, "planes": 5000, "alliance_name": "Second United Front", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 5, "requests": 4, "act_divisions": 38, "act_boats": 50, "act_planes": 5000 },
    { "game": game_obj, "name": "Yemen", "user": closed, "player_number": 84, "states": 1, "divisions": 39, "boats": 50, "planes": 5000, "alliance_name": "", "points": 0, "nuke_time": 8, "nukes": 0, "attacks": 10, "requests": 4, "act_divisions": 39, "act_boats": 50, "act_planes": 5000 },
    ]
